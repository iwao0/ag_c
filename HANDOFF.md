...
現在 `make test` = **1008/1008 E2E green**（unit/parser/preprocess/IR 含む全 green）。
...
1. **ローカルの 2 次元ポインタ配列**（グローバル版は 320e0ff で修正済み）。`int *t[2][2]={{&w,&x},{&y,&z}};`
   をローカルで宣言すると `*t[1][1]` が SIGSEGV、funcptr ローカル `int(*t[2][2])(void)={{a,b},{c,d}}` は
   brace init が E3064（スカラ初期化子の波括弧は 1 要素のみ。ネスト brace init 対象外）。グローバルとは
   **別の表現衝突**で decl.c の登録経路を要修正:
   - データ版: register_multidim_array_lvar（decl.c:2860）は arr_elem_size=8 で outer_stride=16 を立てるが、
     登録後（decl.c:3296〜）に `pointer_qual_levels=1` と `base_deref_size=elem_size(4)` を立てる。
     build_subscript_deref の「要素はポインタ」分岐（pql>=1 && bds>0, expr.c:3237）が **1 段目** `t[1]` で
     発火し、行でなく最終 int*（deref_size=4）扱いになって 2 段目が +4/ldrsw（4B）に化ける（要素は 8B
     ポインタのはず）。グローバルで使った pointee_is_scalar_ptr の中間次元伝播（320e0ff）と整合する形で、
     ローカルの 2D+ ポインタ配列を「多次元 stride を使い最終次元のみポインタ load」にする必要がある。
     1D ローカル `int *arr[N]`（pql=1/bds の既存表現）や非ポインタ多次元に回帰を出さないよう注意。
   - funcptr 版: 加えて `{{a,b},{c,d}}` のネスト brace init をローカル配列初期化で受ける必要がある。
   最小再現:
   ```c
   int w=1,x=2,y=3,z=4;
   int main(void){int *t[2][2]={{&w,&x},{&y,&z}}; return *t[0][0]+*t[1][1]*10;}  /* agc SIGSEGV, clang=41 */
   ```
