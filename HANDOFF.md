...
現在 `make test` = **1003/1003 E2E green**（unit/parser/preprocess/IR 含む全 green）。
...
1. **多段ポインタ戻り** `int **g(); **g()` が SIGSEGV。semantic ctx の ret_is_pointer が bool（段数なし）で
   `int **` を `int *`（pointee 4B）扱い。型付き変数経由は可。pointer LEVELS の追跡が要る。
2. **extern 宣言＋同一TU定義（tag/typedef 基底）** `extern struct S es; struct S es={7};` が E3064 /
