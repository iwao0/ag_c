...
1. **2 次元関数ポインタ配列グローバル** `int(*t[2][2])(void)={{a,b},{c,d}}; t[0][0]()` が SIGSEGV
   （agc=138 vs clang=41）。1 次元の関数ポインタ配列 `int(*ops[2])(int)` は動作（probe 確認済み）。
   2 次元の配列添字→funcall の経路でアドレス計算かディスパッチが壊れている。最小再現:
   ```c
   int a(void){return 1;} int b(void){return 2;} int c(void){return 3;} int d(void){return 4;}
   int(*t[2][2])(void)={{a,b},{c,d}};
   int main(void){return t[0][0]()+t[1][1]()*10;}
   ```
