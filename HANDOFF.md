...
- **メモリ系バグは ASAN で特定**するのが有効だった（間欠 SIGSEGV を 1 回で再現）。
  ...
  ```
  SRCS="<Makefile のリンク対象 .c を全部スペース区切り、messages_ja.c のみ>"
  clang -std=c11 -g -O0 -fsanitize=address -DDIAG_LANG_JA -Isrc ${=SRCS} -o /tmp/ag_c_asan
  ```
- 新規 fixture は exit code を 42 に揃える慣習。登録は `test/test_e2e.c` の
  `{"probes", "<name>", CASE_INT_FILE, "test/fixtures/probes_found_bugs/<name>.c", 42, 0}`。
...
2026-06-18 セッションで上記 11 件を修正済み。下記は現時点で残る既知の未対応。
直近のプローブ（型×文脈の総当たり寄り）はかなり green で、発見コストが上がっている。）
