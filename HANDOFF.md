...
現在 `make test` = **1007/1007 E2E green**（unit/parser/preprocess/IR 含む全 green）。
...
- **unsigned char/short ポインタ経由の zero-extend** `unsigned char* p; p[i]`（c6ac5d9）
  ...
  signed は符号拡張維持。**注**: HANDOFF の「既知の差異」末尾の complex.h 欠如は解消済み（include/complex.h
  が存在）。`1.0i` 虚数サフィックス・`__real__` 等は GNU 拡張で対象外（clang は受理するが追わない）。

### 発見したが未修正（次セッションの着手候補。再現確認済み）
1. **グローバルの 2 次元ポインタ配列**（データ/関数ポインタ両方）が SIGSEGV。`int(*t[2][2])(void)` も
   `int *t[2][2]` も `char *names[2][2]` も `t[i][j]` で落ちる（非ポインタ `int t[2][2]` は動作）。
   **調査済みの根因（部分的に着手したが revert）**:
   - 多次元 stride 適用 `apply_global_multidim_strides`（parser.c:1769）が `!head.is_ptr` ゲートで
     ポインタ要素配列を除外しており、`t[i]` が「ポインタ値として load→[j] で deref」と誤計算される。
     ゲートを外し elem_size=8 で stride を立てると 1 段目の早期 load は直るが、**まだ不十分**:
   - 最終要素の load が 4B（`ldrsw`、pointee int サイズ）になる。要素は 8B ポインタのはず。
     `try_build_global_var_node`（expr.c:3868〜）で 2D ポインタ配列は「多次元 stride
     (outer_stride/inner_deref_size)」と「1D スカラポインタ配列 (pointee_is_scalar_ptr,
     line 3918)」の両表現が同時に立ち、build_subscript_deref で衝突する。要素 load 幅が
     pointee_elem_size(4) になり 8B ポインタを 4B で読む。**両表現の統合が要**（1D ptr 配列・
     非ポインタ多次元の既存経路に回帰を出さないよう注意）。funcptr paren 版 `(*t[2][2])(void)` は
     さらに paren_array_mul が積(4)のみで個別 dims `[2][2]` を捨て 1D 化する別問題も重なる。
   - **ローカルの 2D ポインタ配列**も別途未対応: `int(*t[2][2])(void)={{a,b},{c,d}}` の brace init が
     E3064（スカラ初期化子の波括弧は 1 要素のみ）。ネスト brace 初期化の対象外。
   最小再現:
   ```c
   int w=1,x=2,y=3,z=4; int *t[2][2]={{&w,&x},{&y,&z}};
   int main(void){return *t[0][0]+*t[1][1]*10;}   /* agc SIGSEGV, clang=41 */
   ```
- それ以外は exploratory 差分テスト（/tmp に probe を量産→agc_diff_test）で新規 miscompile を炙り出す。
  索引は `docs/differential_testing/bug_coverage.md`。プローブはかなり green で発見コストが上昇。
