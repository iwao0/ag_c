...
現在 `make test` = **1012/1012 E2E green**（unit/parser/preprocess/IR 含む全 green）。
...
- **欠落 C11 標準ヘッダ 10 個を同梱**（2307f83）c11_standard_headers。iso646/stdalign/stdnoreturn/uchar/
  ...
  **C11 残ギャップ**: UTF-16/32 文字列の非 ASCII（UTF-8 デコード）未対応、long double 制御式の _Generic は
  default に落ちる（ag_c が long double→double lowering、同 ABI なので値は一致）。
