...
### このセッション（続き2）: op(41).v を修正 → 非 pow2 サイズ struct の間接戻りを新規発見
...
1. **非 pow2 サイズ（1/2/4/8B 以外）struct を返す関数ポインタの間接呼び出し**が IR build 失敗。
   `struct Big{int a,b,c,d,e;}(20B)` や 12B struct を返す `struct Big (*ob)(int)=mkbig; ob(100)` が
   「ir build/emit failed」。メンバアクセス以前に `struct Big r=ob(100);`（代入のみ）でも落ちる別経路で、
   op(41).v 修正とは独立。直接呼び出し `mkbig(100).a` は動作。x8 ret_area 間接返し ABI を間接呼び出し
   経路で扱えていない（1/2/4/8B のレジスタ返しは funcptr_return_struct_member で動作確認済み）。
