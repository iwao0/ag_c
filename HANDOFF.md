...
現在 `make test` = **1010/1010 E2E green**（unit/parser/preprocess/IR 含む全 green）。
...
- 現状、HANDOFF 列挙の既知未対応バグはすべて消化済み。再開時は exploratory 差分テスト
  （/tmp に probe を量産→agc_diff_test）で新規 miscompile を炙り出すところから。
