...
現在 `make test` = **1005/1005 E2E green**（unit/parser/preprocess/IR 含む全 green）。
...
- 現状、HANDOFF に列挙していた既知の未修正バグはすべて消化済み。再開時は exploratory 差分テスト
  （/tmp に probe を量産→agc_diff_test）で新しい miscompile / コンパイルエラーを炙り出すところから。
  索引は `docs/differential_testing/bug_coverage.md`。直近のプローブはかなり green で発見コストが上昇。
