...
現在 `make test` = **1009/1009 E2E green**（unit/parser/preprocess/IR 含む全 green）。
...
- **ローカルの 2 次元以上のデータポインタ配列** `int *t[2][2]`（ff709ed）local_2d_pointer_array。
  ...
  網羅。1D `int *arr[N]`・genuine `int **pp` は不変。**funcptr ローカルは別問題で未修正**（下記 1.）。
...
1. **ローカルの 2 次元「関数ポインタ」配列** `int(*t[2][2])(void)`（データポインタ版 `int *t[2][2]` は
   ff709ed、グローバル版は 320e0ff で修正済み）。2 つの問題が重なる:
   - ネスト brace init `{{a,b},{c,d}}` が E3064（スカラ初期化子の波括弧は 1 要素のみ）。ローカル funcptr
     配列の多次元 brace init を受ける必要がある。
   - 要素を個別代入しても (`t[0][0]=a; ...`) `t[i][j]()` が SIGSEGV。ローカルの paren array
     `(*t[2][2])(void)` 経路（decl.c:3224〜 の `else if (paren_array_mul > 0)`）が pointer-to-array
     扱いで多次元 funcptr 配列のストライド（outer=16, 要素=8）を立てない。グローバル paren 版
     （320e0ff の psx_parse_array_suffixes_capture_dims）に相当する局所版が要る。
   最小再現:
   ```c
   int a(void){return 1;} int b(void){return 2;} int c(void){return 3;} int d(void){return 4;}
   int main(void){int(*t[2][2])(void)={{a,b},{c,d}}; return t[0][0]()+t[1][1]()*10;}  /* E3064 */
   ```
