...
double __agc_runtime_pow(double x, double y) {
  ...
  if (x == 0.0) {
    if (y < 0.0) {
      double inf = 1.0 / 0.0;
      return __agc_runtime_signbit(x) && y_is_odd_int ? -inf : inf;
    }
    if (__agc_runtime_signbit(x) && y_is_odd_int) return -0.0;
    return 0.0;
  }
  if (__agc_runtime_isinf(x)) {
    double inf = 1.0 / 0.0;
    if (y > 0.0) return __agc_runtime_signbit(x) && y_is_odd_int ? -inf : inf;
    return __agc_runtime_signbit(x) && y_is_odd_int ? -0.0 : 0.0;
  }
  if (y_is_int) {
    double base = x;
    double result = 1.0;
...
double __agc_runtime_log(double x) {
  ...
  if (__agc_runtime_isinf(x)) return x;
  int k = 0;
  while (x > 2.0) {
    x = x / 2.0;
    k++;
...
static double ag_rt_erfc_tail(double x) {
  double t;
  double poly;
  t = 1.0 / (1.0 + 0.3275911 * x);
  poly = (((((1.061405429 * t - 1.453152027) * t) + 1.421413741) * t -
           0.284496736) * t + 0.254829592) * t;
  return poly * __agc_runtime_exp(-x * x);
}
...
double __agc_runtime_erf(double x) {
  ...
  tail = ag_rt_erfc_tail(x);
  return sign * (1.0 - tail);
}
...
double __agc_runtime_erfc(double x) {
  ...
  if (x == 0.0) return 1.0;
  ax = x < 0.0 ? -x : x;
  tail = ag_rt_erfc_tail(ax);
  return x < 0.0 ? 2.0 - tail : tail;
}
...
double __agc_runtime_sinh(double x) {
  ...
  if (__agc_runtime_isinf(x)) return x;
  if (x == 0.0) return x;
  if (__agc_runtime_fabs(x) < 1.0e-4) return x;
  ex = __agc_runtime_exp(x);
  em = __agc_runtime_exp(-x);
  return (ex - em) / 2.0;
}
