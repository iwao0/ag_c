...
function parseSignature(signature, label) {
  ...
  if (params.length > 16) fail(`${label} has more than 16 parameters`);
...
function generateC(manifest) {
  ...
  for (const entry of manifest.functions.filter((item) => item.bridge !== "host")) {
    const signature = cSignature(entry.signature);
