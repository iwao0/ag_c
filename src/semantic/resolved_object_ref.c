...
static node_lvar_t *new_lvar_symbol_node(
  ...
  psx_resolved_reference_state_t *reference =
      reference_state((node_t *)node);
  if (reference) {
    reference->kind = PSX_RESOLVED_REFERENCE_LOCAL;
    reference->local = var;
    reference->storage_offset = offset;
  }
  if (var) {
...
node_t *ps_node_new_gvar_array_addr_for_in(
  ...
  node_t *address = new_address_node(
      arena_context,
      psx_node_new_gvar_array_base_for_in(arena_context, global));
  bind_array_address_type(
      arena_context, address, ps_gvar_get_decl_type(global));
  return address;
}
...
node_t *psx_node_new_static_local_array_addr_for_in(
  ...
  const psx_type_t *backing_type =
      var && var->static_global
          ...
          : NULL;
  bind_array_address_type(arena_context, address, backing_type);
  return address;
}
...
node_t *ps_node_new_lvar_array_addr_for_in(
  ...
  bind_array_address_type(
      arena_context, address, ps_lvar_get_decl_type(var));
  return address;
}
...
static node_gvar_t *new_global_symbol_node(
    arena_context_t *arena_context) {
  node_gvar_t *node = psx_resolution_node_alloc_in(
      arena_context, sizeof(*node));
  if (node) node->base.kind = ND_GVAR;
  return node;
}

node_t *ps_node_new_gvar_for_in(
    arena_context_t *arena_context, global_var_t *global) {
  node_gvar_t *node = new_global_symbol_node(arena_context);
  if (!node) return NULL;
  if (global) {
    node->symbol = global;
    ps_node_bind_qual_type(
        (node_t *)node, ps_gvar_get_decl_type(global),
        ps_gvar_decl_qual_type(global));
    node->name = global->name;
    node->name_len = global->name_len;
    node->is_thread_local = global->is_thread_local ? 1 : 0;
  }
  psx_resolved_reference_state_t *reference =
      reference_state((node_t *)node);
  if (reference) {
...
node_t *psx_node_new_gvar_array_base_for_in(
    arena_context_t *arena_context, global_var_t *global) {
  return ps_node_new_gvar_for_in(arena_context, global);
}
...
node_t *psx_node_new_function_reference_in(
  ...
  ps_node_bind_type(
      ...
      function_type
          ...
          : NULL);
  reference->funcname = name;
  reference->funcname_len = name_len;
  psx_resolved_reference_state_t *state =
      reference_state((node_t *)reference);
  if (state) {
...
node_t *psx_node_new_va_arg_area_reference_in(
  ...
  if (node) {
    ...
    if (state)
      state->kind = PSX_RESOLVED_REFERENCE_VA_ARG_AREA;
  }
  return node;
}

lvar_t *ps_node_lvar_symbol(node_t *node) {
  return psx_resolved_object_ref_local(node);
}
...
psx_resolved_object_ref_kind_t psx_resolved_object_ref_kind(
  ...
  switch (state->reference.kind) {
    ...
    case PSX_RESOLVED_REFERENCE_VA_ARG_AREA:
      return PSX_RESOLVED_OBJECT_REF_VA_ARG_AREA;
