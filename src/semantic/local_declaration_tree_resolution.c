...
typedef struct {
  ...
  psx_lowering_context_t *lowering_context;
  const ag_compilation_options_t *options;
} psx_local_declaration_application_context_t;
...
static void *begin_declaration(
  ...
  const psx_local_declaration_application_context_t *application_context =
      context;
  psx_local_declaration_application_t *application =
      calloc(1, sizeof(*application));
  if (!application) {
    ps_diag_ctx_in(
        ps_ctx_diagnostics(application_context->semantic_context),
        specifier ? specifier->diagnostic_token : NULL,
        "local-declaration", "local declaration allocation failed");
  }
  application->semantic_context = application_context->semantic_context;
  application->global_registry = application_context->global_registry;
  application->local_registry = application_context->local_registry;
  application->lowering_context = application_context->lowering_context;
  application->options = application_context->options;
...
static node_t *apply_local_declaration_syntax(
  ...
  void *application = begin_declaration(
      &application_context, &declaration->specifier,
