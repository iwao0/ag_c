...
void psx_apply_aggregate_member_layout_metadata(
  ...
  if (metadata->outer_stride > 0 && type->outer_stride <= 0)
    type->outer_stride = metadata->outer_stride;
  if (metadata->mid_stride > 0 && type->mid_stride <= 0)
    type->mid_stride = metadata->mid_stride;
  if (metadata->ptr_array_pointee_bytes <= 0) return;
  type->ptr_array_pointee_bytes = metadata->ptr_array_pointee_bytes;
  if (type->outer_stride <= 0)
    type->outer_stride = metadata->ptr_array_pointee_bytes;
  if (type->kind == PSX_TYPE_POINTER) {
    type->deref_size = metadata->ptr_array_pointee_bytes;
    if (type->base_deref_size <= 0 && metadata->deref_size > 0)
      type->base_deref_size = metadata->deref_size;
  }
  if (type->kind == PSX_TYPE_ARRAY && type->base)
    psx_apply_aggregate_member_layout_metadata(type->base, metadata);
}
...
psx_type_t *psx_resolve_aggregate_member_type(
  ...
  if (!type) return NULL;
  if (request->has_funcptr_signature &&
      ps_decl_funcptr_sig_has_payload(request->funcptr_signature)) {
    type = psx_type_attach_funcptr_signature(
        type, request->funcptr_signature);
  }
  psx_apply_aggregate_member_layout_metadata(
      type, request->layout_metadata);
  return type;
}
...
void psx_resolve_aggregate_bitfield_placement(
  ...
  if (!state || !request || !is_aggregate_kind(state->kind) ||
      request->storage_size <= 0 || request->storage_size > 8 ||
      request->bit_width < 0) {
    return;
  }
  int storage_size = request->storage_size;
...
void psx_resolve_aggregate_member(
  ...
  if (!request || !is_aggregate_kind(request->tag_kind) ||
      !request->tag_name || request->tag_name_len <= 0 || !request->member ||
      !request->member->name || request->member->len < 0 ||
      request->member->offset < 0 || !ps_tag_member_decl_type(request->member)) {
    return;
  }
  int created = 0;
  if (!psx_ctx_register_tag_member(
          request->tag_kind, request->tag_name, request->tag_name_len,
          request->member, &created)) {
    resolution->status = request->member->len > 0
