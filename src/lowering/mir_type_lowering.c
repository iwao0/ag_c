...
static int integer_size_for_rank(int rank,
                                 const ag_data_layout_t *data_layout) {
  return ag_data_layout_scalar_size(data_layout, scalar_kind_for_rank(rank));
}

int ir_mir_integer_promotion_is_unsigned(ir_mir_type_info_t type,
                                         const ag_data_layout_t *data_layout) {
  if (type.type_class != IR_MIR_TYPE_INTEGER ||
      !ag_data_layout_is_valid(data_layout) || type.integer_rank <= 0)
    return 0;
  if (type.integer_rank >= 3) return type.is_unsigned;
  return type.is_unsigned &&
         integer_size_for_rank(type.integer_rank, data_layout) >=
...
static ir_mir_integer_conversion_t
promoted_integer(ir_mir_type_info_t type, const ag_data_layout_t *data_layout) {
  return (ir_mir_integer_conversion_t){
      .rank = type.integer_rank < 3 ? 3 : type.integer_rank,
      .is_unsigned = ir_mir_integer_promotion_is_unsigned(type, data_layout),
  };
}
...
int ir_mir_usual_arithmetic_result_is_unsigned(
  ...
  if (left.type_class != IR_MIR_TYPE_INTEGER ||
      right.type_class != IR_MIR_TYPE_INTEGER ||
      !ag_data_layout_is_valid(data_layout))
    return 0;
  ir_mir_integer_conversion_t lhs = promoted_integer(left, data_layout);
  ir_mir_integer_conversion_t rhs = promoted_integer(right, data_layout);
  ...
  return integer_size_for_rank(signed_type.rank, data_layout) <=
