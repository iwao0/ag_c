...
int tk_read_escape_char(char **pp) {
  ...
  if (*p == 'x' && !tk_is_xdigit(p[1])) {
    tk_error_at_id(DIAG_ERR_TOKENIZER_INVALID_ESCAPE_HEX, p + 1, "16進エスケープが不正です");
  }
  if (*p == 'u' || *p == 'U') {
    uint32_t cp = 0;
    int consumed = 0;
    if (!tk_parse_ucn_codepoint(p - 1, &cp, &consumed)) {
      tk_error_at_id(DIAG_ERR_TOKENIZER_INVALID_ESCAPE_UCN, p, "UCNエスケープが不正です");
    }
  ...
  switch (*p) {
    ...
    case '0': case '1': case '2': case '3': case '4': case '5': case '6': case '7':
      break;
    default:
      tk_error_at_id(DIAG_ERR_TOKENIZER_INVALID_ESCAPE_GENERAL, p, "不正なエスケープです");
  }
  ...
  *has_ucn = false;
  ...
  for (int i = 0; i < len;) {
    ...
    if (tk_parse_ucn_codepoint(start + i, &cp, &consumed)) {
      if (!tk_is_valid_ucn_codepoint(cp))
        tk_error_at_id(DIAG_ERR_TOKENIZER_INVALID_ESCAPE_UCN, start + i,
                       "識別子内のUCNが不正です");
      char tmp[4];
