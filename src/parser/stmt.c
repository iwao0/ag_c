...
static node_t *stmt_internal(psx_statement_parse_context_t *context) {
  ...
  /* 式文 (式を評価して結果を捨てる) */
  node_t *node = psx_expr_expr_in_context(
      context->semantic_context, context->local_declarations);
  tk_expect(';');
  return node;
}
...
static node_t *parse_stmt_return(
  ...
  node->lhs = psx_expr_expr_in_context(
      context->semantic_context, context->local_declarations);
  tk_expect(';');
  return node;
}
...
static node_t *parse_stmt_if(psx_statement_parse_context_t *context) {
  ...
  node->base.lhs = psx_expr_expr_in_context(
      context->semantic_context, context->local_declarations);
  tk_expect(')');
...
static node_t *parse_stmt_for(psx_statement_parse_context_t *context) {
  ...
  if (!tk_consume(';')) {
    if (is_decl_like_start_stmt(context)) {
      ...
      node->init = psx_expr_expr_in_context(
          context->semantic_context, context->local_declarations);
      tk_expect(';');
    }
  }
  ...
  if (!tk_consume(')')) {
    node->inc = psx_expr_expr_in_context(
        context->semantic_context, context->local_declarations);
    tk_expect(')');
  }
  node->base.rhs = stmt_internal(context);
