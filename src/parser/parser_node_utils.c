...
node_t *pnode_new_lvar_typed(int offset, int type_size) {
  node_lvar_t *node = (node_lvar_t *)pnode_new_lvar(offset);
...
node_t *pnode_new_compound_assign(node_t *lhs, node_kind_t op_kind, node_t *rhs, const char *op) {
  pnode_expect_lvalue(lhs, op);
  node_t *op_expr = pnode_new_binary(op_kind, lhs, rhs);
  node_mem_t *assign_node = pnode_new_assign(lhs, op_expr);
  assign_node->type_size = pnode_type_size(lhs);
