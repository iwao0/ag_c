...
static unsigned pctx_hash_name(const char *name, int len) {
  // djb2 variant
  unsigned h = 5381u;
  for (int i = 0; i < len; i++) {
...
static unsigned pctx_hash_tag(token_kind_t kind, const char *name, int len) {
...
void pctx_validate_goto_refs(void) {
  for (goto_ref_t *g = goto_refs_all; g; g = g->next_all) {
    unsigned bucket = pctx_hash_name(g->name, g->len);
    int found = 0;
    for (label_def_t *d = label_defs_by_bucket[bucket]; d; d = d->next_hash) {
...
bool pctx_has_tag_type(token_kind_t kind, char *name, int len) {
  unsigned bucket = pctx_hash_tag(kind, name, len);
