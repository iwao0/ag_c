...
static int gvar_member_value_size_for_target(
  ...
  return ps_type_sizeof_id(semantic_types, record_layouts, value_type_id,
...
static int gvar_member_storage_size_for_target(
  ...
  return ps_type_sizeof_id(semantic_types, record_layouts, member_type_id,
                           ag_target_info_data_layout(target));
}
...
static int record_member_declaration_storage_size_in(
    psx_semantic_context_t *semantic_context,
    const psx_record_member_decl_t *member) {
  return member ? ps_type_sizeof_id(
                      ps_ctx_semantic_type_table_in(semantic_context),
                      ps_ctx_record_layout_table_in(semantic_context),
...
static int gvar_init_cursor_pack_bitfield_unit(
  ...
  int unit_size =
      ps_type_sizeof_id(semantic_types, record_layouts, unit_type_id,
                        ag_target_info_data_layout(target));
...
int ps_tag_select_union_member_for_init_slot_in(
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (!ps_ctx_get_tag_member_in(
      ...
      break;
    int cand_fp_size = record_member_decl_fp_size(
        ps_ctx_semantic_type_table_in(semantic_context), &candidate);
...
static int tag_union_init_member_for_slot_scoped(
  ...
  if (record_decl) {
    ...
    *out_declaration = record_decl->members[ordinal];
    int init_fp_size = ps_gvar_union_init_slot_fp_size(gv, idx);
    int selected_fp_size = record_member_decl_fp_size(
        ps_ctx_semantic_type_table_in(semantic_context), out_declaration);
