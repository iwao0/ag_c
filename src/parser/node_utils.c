...
static int lvar_decl_type_stale_for_fields(const lvar_t *var, const psx_type_t *type) {
  if (!var || !type) return 0;
  if (psx_ctx_is_tag_aggregate_kind(var->tag_kind) && !var->is_tag_pointer &&
      type_tag_aggregate_kind(type) != var->tag_kind) {
    return 1;
  }
  if (var->is_array && type->kind != PSX_TYPE_ARRAY) return 1;
  if (!var->is_array && var->pointer_qual_levels >= 1 &&
  ...
  if (!var->is_array && var->pointer_qual_levels >= 1 &&
      var->ptr_array_pointee_bytes > 0 && var->elem_size == 8 &&
      var->base_deref_size > 0 && var->base_deref_size < 8 &&
      (var->ptr_array_pointee_bytes % 8) == 0 &&
      type->kind == PSX_TYPE_POINTER && type->base &&
      type->base->kind == PSX_TYPE_ARRAY && type->base->elem_size != 8) {
    return 1;
  }
  return 0;
}

static int gvar_decl_type_stale_for_fields(const global_var_t *gv, const psx_type_t *type) {
  if (!gv || !type) return 0;
  if (psx_ctx_is_tag_aggregate_kind(gv->tag_kind) && !gv->is_tag_pointer &&
      type_tag_aggregate_kind(type) != gv->tag_kind) {
    return 1;
  }
  if (gv->is_array && type->kind != PSX_TYPE_ARRAY) return 1;
  if (!gv->is_array && gv->pointer_qual_levels >= 1 &&
  ...
  if (!gv->is_array && gv->pointer_qual_levels >= 1 &&
      gv->ptr_array_pointee_bytes > 0 && gv->deref_size == 8 &&
      gv->pointee_elem_size > 0 && gv->pointee_elem_size < 8 &&
      (gv->ptr_array_pointee_bytes % 8) == 0 &&
      type->kind == PSX_TYPE_POINTER && type->base &&
      type->base->kind == PSX_TYPE_ARRAY && type->base->elem_size != 8) {
    return 1;
  }
...
static psx_type_t *lvar_decl_type_consistent(lvar_t *var) {
  ...
  if (lvar_decl_type_stale_for_fields(var, type)) {
    psx_decl_invalidate_lvar_decl_type(var);
    type = psx_lvar_materialize_decl_type(var);
  }
  return type;
}
...
static psx_type_t *gvar_decl_type_consistent(global_var_t *gv) {
  ...
  if (gvar_decl_type_stale_for_fields(gv, type)) {
    psx_decl_invalidate_gvar_decl_type(gv);
    type = psx_gvar_materialize_decl_type(gv);
  }
  return type;
}
...
static int gvar_walk_struct_initializer(token_kind_t tag_kind, char *tag_name, int tag_len,
  ...
  while (gvar_init_cursor_has(cur)) {
      ...
      if (psx_tag_member_is_tag_aggregate(&mi)) {
        for (int k = 0; k < mi.array_len; k++) {
          ...
          int ok = psx_tag_member_is_union_aggregate(&mi)
              ? gvar_walk_union_initializer(mi.tag_kind, mi.tag_name, mi.tag_len,
                                            0,
                                            gv, cur, elem_off, mi.type_size,
                                            ops, user)
              : gvar_walk_struct_initializer(mi.tag_kind, mi.tag_name, mi.tag_len,
                                             0,
                                             gv, cur, elem_off, mi.type_size,
                                             ops, user);
          if (!ok) return 0;
          gvar_init_cursor_consume_tag_zero_padding(mi.tag_kind, mi.tag_name,
                                                    mi.tag_len, cur,
                                                    elem_start_idx);
        }
      } else {
    ...
    if (psx_tag_member_is_struct_aggregate(&mi)) {
      ...
      if (!gvar_walk_struct_initializer(mi.tag_kind, mi.tag_name, mi.tag_len, 0,
    ...
    if (psx_tag_member_is_union_aggregate(&mi)) {
      if (!gvar_walk_union_initializer(mi.tag_kind, mi.tag_name, mi.tag_len, 0,
                                       gv, cur, base_offset + mi.offset,
                                       mi.type_size,
                                       ops, user)) {
        return 0;
...
static void mem_from_gvar(node_mem_t *mem, global_var_t *gv) {
  *mem = (node_mem_t){0};
  ...
  mem->is_tag_pointer = (gv->is_tag_pointer && gv->ptr_array_pointee_bytes <= 0) ? 1 : 0;
...
static psx_type_t *type_from_funcptr_callee_type(node_func_t *fn) {
  ...
  int has_tag_value_return =
      !callee_sig.ret_is_data_pointer && psx_ctx_is_tag_aggregate_kind(tag_kind);
...
psx_decl_funcptr_sig_t psx_lvar_funcptr_sig(const lvar_t *src) {
  ...
  if (type) sig = funcptr_sig_merge_missing(sig, &type->funcptr_sig, 1);
  psx_decl_funcptr_sig_t raw = funcptr_sig_from_lvar(src);
  return funcptr_sig_merge_missing(sig, &raw, 1);
}
...
psx_decl_funcptr_sig_t psx_gvar_funcptr_sig(const global_var_t *src) {
  ...
  if (type) sig = funcptr_sig_merge_missing(sig, &type->funcptr_sig, 1);
  psx_decl_funcptr_sig_t raw = funcptr_sig_from_gvar(src);
  return funcptr_sig_merge_missing(sig, &raw, 1);
}
...
static int gvar_ref_deref_size_from_type(const global_var_t *gv,
  ...
  if (decl_type->kind == PSX_TYPE_POINTER && decl_type->outer_stride > 0) {
    if (gv && gv->outer_stride > decl_type->outer_stride) return gv->outer_stride;
    return decl_type->outer_stride;
  }
  int deref_size = psx_type_deref_size(decl_type);
  if (gv && gv->outer_stride > deref_size &&
      decl_type->kind == PSX_TYPE_POINTER &&
      (decl_type->mid_stride > 0 || decl_type->extra_strides_count > 0)) {
    return gv->outer_stride;
  }
  return deref_size;
}
...
static void apply_array_addr_decl_type(node_mem_t *addr, psx_type_t *array_type) {
  ...
  if (base_deref_size > 0 && addr->base_deref_size == 0)
...
node_t *psx_node_new_lvar_fp_slot_for(lvar_t *owner, int offset, int type_size) {
  ...
  node->var = owner;
  if (owner && owner->fp_kind != TK_FLOAT_KIND_NONE)
    node->mem.base.type = psx_type_new_float(owner->fp_kind, type_size);
  return (node_t *)node;
}
...
node_t *psx_node_new_lvar_for(lvar_t *var) {
  ...
  if (var) {
    mem_from_lvar(&node->mem, var);
    node->mem.base.type = psx_lvar_get_decl_type(var);
    sync_scalar_mem_from_decl_type(&node->mem, node->mem.base.type);
    sync_pointer_cast_mem_from_type(&node->mem, node->mem.base.type);
  }
  node->var = var;
...
static int lvar_identifier_deref_size_from_type(const lvar_t *var,
  ...
  if (decl_type->kind == PSX_TYPE_POINTER && decl_type->outer_stride > 0) {
    if (var && var->outer_stride > decl_type->outer_stride)
      return var->outer_stride;
    return decl_type->outer_stride;
  }
  ...
  int deref_size = psx_type_deref_size(decl_type);
  if (var && var->outer_stride > deref_size &&
      (decl_type->kind == PSX_TYPE_ARRAY || decl_type->kind == PSX_TYPE_POINTER) &&
      (decl_type->is_vla || var->is_vla || decl_type->mid_stride > 0 ||
       decl_type->extra_strides_count > 0)) {
    return var->outer_stride;
  }
  return deref_size;
}
...
static void sync_lvar_identifier_mem_from_decl_type(node_lvar_t *node,
  ...
  } else if (var && (var->is_vla || var->outer_stride > 0)) {
  ...
  node->mem.pointer_qual_levels = (unsigned char)psx_node_pointer_qual_levels((node_t *)node);
  node->mem.ptr_array_pointee_bytes =
      decl_type->ptr_array_pointee_bytes > 0
          ? decl_type->ptr_array_pointee_bytes
          : (var ? var->ptr_array_pointee_bytes : 0);
  node->mem.vla_row_stride_frame_off =
      decl_type->vla_row_stride_frame_off != 0
          ? decl_type->vla_row_stride_frame_off
          : (var ? var->vla_row_stride_frame_off : 0);
  node->mem.vla_strides_remaining =
      decl_type->vla_strides_remaining > 0
          ? decl_type->vla_strides_remaining
          : (var ? var->vla_strides_remaining : 0);
}
...
static void sync_pointer_cast_mem_from_type(node_mem_t *mem, psx_type_t *type) {
  ...
  const psx_type_t *pointee_value_type = type_pointee_value_type(type);
  if (pointee_value_type &&
      (pointee_value_type->kind == PSX_TYPE_STRUCT ||
       pointee_value_type->kind == PSX_TYPE_UNION)) {
    mem->tag_kind = pointee_value_type->tag_kind;
    mem->tag_name = pointee_value_type->tag_name;
    mem->tag_len = pointee_value_type->tag_len;
    mem->tag_scope_depth_p1 = pointee_value_type->tag_scope_depth_p1;
    mem->is_tag_pointer = 1;
  }
}
...
static psx_type_t *type_decay_array_to_pointer(psx_type_t *array_type) {
  ...
  ptr->funcptr_sig = array_type->funcptr_sig;
...
static psx_type_t *type_from_subscript_base_type(const psx_type_t *base_type,
  ...
  row->funcptr_sig = view->funcptr_sig;
...
node_t *psx_node_new_tag_member_deref_for(node_t *addr_base, node_t *base,
  ...
  if (psx_decl_funcptr_sig_has_payload(member_funcptr_sig) &&
      member_funcptr_sig.ret_fp_kind == TK_FLOAT_KIND_NONE &&
      !member_funcptr_sig.ret_is_data_pointer &&
      info->fp_kind != TK_FLOAT_KIND_NONE) {
    member_funcptr_sig.ret_fp_kind = info->fp_kind;
  }
...
static int tag_member_ref_deref_size_from_type(const tag_member_info_t *info,
  ...
  int deref_size = psx_type_deref_size(type);
  if (info && info->outer_stride > deref_size) return info->outer_stride;
  return deref_size;
}
...
static void sync_tag_member_mem_from_decl_type(node_mem_t *mem,
  ...
  if (has_stride) {
    if (type->kind == PSX_TYPE_POINTER &&
        info && info->outer_stride > info->deref_size &&
        info->mid_stride <= 0 && info->deref_size > 0) {
      inner_stride = info->deref_size;
      next_stride = 0;
      extra_count = 0;
      for (int i = 0; i < 5; i++) extra_strides[i] = 0;
    }
