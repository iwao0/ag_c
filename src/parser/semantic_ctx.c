...
psx_qual_type_t ps_ctx_intern_enum_qual_type_in(
  ...
  if (!declaration || !tag || tag->kind != TK_ENUM)
    return (psx_qual_type_t){PSX_TYPE_ID_INVALID,
                             PSX_TYPE_QUALIFIER_NONE};
...
psx_semantic_context_t *ps_ctx_create(
    ...
    psx_scope_graph_t *scope_graph,
    const ag_target_info_t *target) {
  if (!arena_context || !diagnostic_context || !scope_graph ||
...
int ps_ctx_mark_tag_record_anonymous_in(
    psx_semantic_context_t *context,
    token_kind_t kind, char *name, int len) {
  psx_record_decl_t *record = (psx_record_decl_t *)
      ps_ctx_ensure_tag_record_decl_in(context, kind, name, len);
  if (!record) return 0;
...
static int register_tag_members_for_owner_in(
  ...
  for (int i = 0; i < member_count; i++) {
    ...
    if (!declaration->name || declaration->len < 0 ||
        !psx_semantic_type_table_qual_type_is_valid(
            context->semantic_types, declaration->decl_qual_type) ||
        layout->offset < 0 ||
        layout->bit_offset < 0) {
  ...
  if (!psx_scope_graph_checkpoint_begin(
          context->scope_graph, &checkpoint))
    return 0;
  for (int i = 0; i < member_count; i++) {
    const psx_record_member_decl_t *declaration = &declarations[i];
    if ((declaration->len > 0 &&
         psx_scope_graph_lookup_in_scope(
             context->scope_graph, tag->member_scope_id,
             PSX_NAMESPACE_MEMBER, declaration->name,
             declaration->len) != PSX_DECL_ID_INVALID) ||
        !insert_tag_member_record_in(
  ...
  psx_scope_graph_checkpoint_commit(&checkpoint);
...
bool psx_ctx_find_typedef_layout_in(
  ...
  if (out_size)
    *out_size =
        psx_qual_type_layout_sizeof(
            context->semantic_types, context->record_layouts,
            qual_type, ps_ctx_data_layout(context));
  if (out_alignment)
    *out_alignment =
        psx_qual_type_layout_alignof(
            context->semantic_types, context->record_layouts,
            qual_type, ps_ctx_data_layout(context));
  return true;
}
...
int ps_function_symbol_has_explicit_extern(
    const psx_function_symbol_t *symbol) {
  return symbol && symbol->has_explicit_extern;
}
...
int ps_ctx_mark_function_explicit_extern_in(
  ...
  psx_function_symbol_t *function =
      find_function_name_mut_in(context, name, len);
  if (!function) return 0;
  function->has_explicit_extern = 1;
  return 1;
}
