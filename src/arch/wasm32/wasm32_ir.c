...
static void wasm_unsupported_inst(
    wasm32_ir_context_t *context,
...
static int has_minimal_libc_stub_function(char *name, int name_len) {
  static const char *const stub_names[] = {
      ...
      "cbrtf", "cbrtl", "ceil", "ceilf", "ceill", "clearerr", "clock", "close",
      ...
      "lrintf", "lrintl", "lround", "lroundf", "lroundl", "lseek", "malloc", "mblen",
      ...
      "nearbyint", "nearbyintf", "nearbyintl", "open", "perror", "pow", "powf", "powl", "printf", "putc",
      "putchar", "puts", "putwc", "putwchar", "qsort", "raise", "rand",
      "quick_exit", "read", "realloc", "realpath", "remainder", "remainderf", "remainderl", "remove",
      ...
      "wcstoimax", "wcstol", "wcstold", "wcstoll", "wcstombs", "wcstoul",
      "wcstoull", "wcstoumax",
      "wcsxfrm", "wctob", "wctomb", "wctrans", "wctype", "wmemchr", "wmemcmp",
      "wmemcpy", "wmemmove", "wmemset", "wprintf", "wscanf", "write", "fwprintf",
      "fwscanf",
  };
...
static void emit_atomic_load_expr(
  ...
  if (!op) wasm_unsupported_inst(context, i);
  wasm_cg_emitf("(%s ", op);
  emit_addr_expr(ctx, i->src1);
  wasm_cg_emitf(")");
}
...
static void emit_atomic_inst(
  ...
  if (selected->atomic.kind == WASM32_MACHINE_ATOMIC_EXCHANGE ||
    ...
    if (!store_op) wasm_unsupported_inst(context, i);
...
static void emit_parameter_bind(
  ...
  if (selected->kind != WASM32_MACHINE_INST_PARAMETER_BIND)
    wasm_unsupported_inst(context, instruction);
  ...
  for (int i = 0; i < binding->piece_count; i++) {
    ...
    if (!store) wasm_unsupported_inst(context, instruction);
    wasm_emitf(indent, "(%s ", store);
...
static void emit_call(
  ...
  if (i->callee.id != IR_VAL_NONE) {
    ...
    if (returns_direct_aggregate) {
      ...
      if (!store) wasm_unsupported_inst(context, i);
      wasm_emitf(indent, "(%s ", store);
...
static void emit_unary(
  ...
  if (!opcode) wasm_unsupported_inst(context, instruction);
  wasm_emitf(
...
static void emit_inst(
  ...
  switch (planned->kind) {
    ...
    case WASM32_MACHINE_INST_STRING_ADDRESS: {
      ...
      if (addr < 0) wasm_unsupported_inst(context, i);
    ...
    case WASM32_MACHINE_INST_CONVERSION: {
      ...
      if (selected->opcode != WASM32_MI_COPY) {
        const char *opcode = wasm32_machine_opcode_wat(selected->opcode);
        if (!opcode) wasm_unsupported_inst(context, i);
        wasm_cg_emitf("(%s ", opcode);
      }
      emit_val_expr(ctx, i->src1);
    ...
    case WASM32_MACHINE_INST_CONTROL:
      switch (planned->control.kind) {
        case WASM32_MACHINE_CONTROL_LABEL:
          if (!dispatch_mode) wasm_unsupported_inst(context, i);
          return;
