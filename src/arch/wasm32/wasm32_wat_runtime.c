...
static void emit_wasm_swprintf_stub(wasm32_ir_context_t *context) {
    ...
    wasm_emitf(6, "(local.set $norm (f64.mul (f64.sub (local.get $norm) (f64.convert_i32_u (local.get $digit))) (f64.const 10)))\n");
    wasm_emitf(6, "(local.set $index (i32.add (local.get $index) (i32.const 1)))\n");
    ...
    wasm_emitf(4, "(local.set $norm (f64.mul (f64.sub (local.get $norm) (f64.convert_i32_u (local.get $digit))) (f64.const 10)))\n");
    wasm_emitf(4, "(if (i32.or (i32.ne (local.get $precision) (i32.const 0)) (local.get $alternate)) (then\n");
    wasm_emitf(6, "(local.set $pos (call $__ag_snwputc (local.get $buf) (local.get $size) (local.get $pos) (i32.const 46)))\n");
    wasm_emitf(4, "))\n");
    wasm_emitf(4, "(local.set $index (i32.const 0))\n");
