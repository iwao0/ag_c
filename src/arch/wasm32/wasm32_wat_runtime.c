...
static void emit_wasm_swprintf_stub(wasm32_ir_context_t *context) {
    ...
    wasm_emitf(6, "(then\n");
    wasm_emitf(8, "(local.set $norm (local.get $rounded))\n");
    wasm_emitf(8, "(local.set $exp (i32.const 0))\n");
    wasm_emitf(8, "(block $norm_done (loop $norm_loop\n");
    wasm_emitf(10, "(if (f64.lt (local.get $norm) (f64.const 10)) (then (br $norm_done)))\n");
    ...
    wasm_emitf(10, "(br $norm_loop)\n");
    wasm_emitf(8, "))\n");
    wasm_emitf(8, "(local.set $index (local.get $exp))\n");
    wasm_emitf(8, "(block $integer_done (loop $integer_loop\n");
    wasm_emitf(10, "(local.set $digit (i32.trunc_f64_u (f64.floor (local.get $norm))))\n");
    wasm_emitf(10, "(local.set $pos (call $__ag_snwputc (local.get $buf) (local.get $size) (local.get $pos) (i32.add (local.get $digit) (i32.const 48))))\n");
    wasm_emitf(10, "(local.set $norm (f64.mul (f64.sub (local.get $norm) (f64.convert_i32_u (local.get $digit))) (f64.const 10)))\n");
    wasm_emitf(10, "(if (i32.eqz (local.get $index)) (then (br $integer_done)))\n");
    wasm_emitf(10, "(local.set $index (i32.sub (local.get $index) (i32.const 1)))\n");
    wasm_emitf(10, "(br $integer_loop)\n");
    wasm_emitf(8, "))\n");
    wasm_emitf(6, ")\n");
    wasm_emitf(6, "(else\n");
    ...
    wasm_emitf(6, "(br $scale_loop)\n");
    wasm_emitf(4, "))\n");
    wasm_emitf(4, "(local.set $norm (call $__ag_snwround_decimal (local.get $norm) (local.get $scale)))\n");
    wasm_emitf(4, "(if (f64.ge (local.get $norm) (f64.const 10)) (then\n");
    wasm_emitf(6, "(local.set $norm (f64.div (local.get $norm) (f64.const 10)))\n");
    ...
    wasm_emitf(6, "(br $scale_loop)\n");
    wasm_emitf(4, "))\n");
    wasm_emitf(4, "(if (f64.ge (call $__ag_snwround_decimal (local.get $norm) (local.get $scale)) (f64.const 10)) (then\n");
    wasm_emitf(6, "(local.set $exp (i32.add (local.get $exp) (i32.const 1)))\n");
    wasm_emitf(4, "))\n");
