...
int main(void) {
  // グローバル array-of-struct メンバ: 各要素を個別に検査
  assert(gshape.pts[0].x == 10);
  assert(gshape.pts[0].y == 20);
  assert(gshape.pts[1].x == 30);
  assert(gshape.pts[1].y == 40);
  assert(gshape.pts[2].x == 0);   // zero-fill
  assert(gshape.pts[2].y == 0);
  assert(gshape.color == 5);

  // ローカル: designated 要素 + 内側 struct designator
  struct Shape ls = { .pts = {[0] = {.x=1, .y=2}, [2] = {.x=5, .y=6}}, .color = 7 };
  assert(ls.pts[0].x == 1);
  assert(ls.pts[0].y == 2);
  assert(ls.pts[1].x == 0);   // zero-fill
  assert(ls.pts[1].y == 0);
  assert(ls.pts[2].x == 5);
  assert(ls.pts[2].y == 6);
  assert(ls.color == 7);
