...
int main(void) {
    ...
    if (a[0].x != 1 || a[0].y != 2 || a[1].x != 3 || a[1].y != 4) return 1;

    // 明示 brace / 互換コピーは従来どおり
    struct P b[2] = {{5, 6}, {7, 8}};
    if (b[1].x != 7 || b[1].y != 8) return 2;
    struct P p0 = {9, 10};
    struct P c[2] = {p0, {11, 12}};
    if (c[0].x != 9 || c[1].y != 12) return 3;

    // (B) 配列メンバ部分省略 → designator 上書き (clang と一致する挙動)
    struct B s = {1, 2, 3, .z = 9};      // a={1,2,3}, z=9
    if (s.a[0] != 1 || s.a[1] != 2 || s.a[2] != 3 || s.z != 9) return 4;

    return a[1].x + a[1].y + b[1].x + c[1].y + s.z + 7;  // 3+4+7+12+9+7 = 42
}
