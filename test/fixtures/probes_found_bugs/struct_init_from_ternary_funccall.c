...
int main(void) {
  ...
  if (s1.code != 0 || s1.flags != 1) r |= 1;
  struct Status s2 = (1 < 0) ? ok() : err(7);
  if (s2.code != 7 || s2.flags != 0) r |= 2;

  // funccall + lvar 混在
  struct Status a = {5, 6};
  struct Status s3 = (1 > 0) ? ok() : a;
  if (s3.code != 0 || s3.flags != 1) r |= 4;
  struct Status s4 = (1 < 0) ? ok() : a;
  if (s4.code != 5 || s4.flags != 6) r |= 8;

  // 両分岐 lvar (従来から対応、回帰確認)
  struct Status b = {3, 4};
  struct Status s5 = (1 > 0) ? a : b;
  if (s5.code != 5 || s5.flags != 6) r |= 16;

  // ネスト three-way
  int sel = 2;
  struct Status s6 = (sel == 1) ? ok() : (sel == 2) ? err(9) : a;
  if (s6.code != 9 || s6.flags != 0) r |= 32;
