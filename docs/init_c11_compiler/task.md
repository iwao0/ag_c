...
- [ ] セキュリティ追加候補（2026-03-23 棚卸し）
  - [x] `##` トークン連結後に再トークナイズ結果が単一トークンであることを検証し、複数トークン化/空トークン化は診断で拒否する
  - [ ] マクロ展開ステップ数に上限を設け、過剰再帰・過剰入れ子でDoS的に時間消費しないことをテストで保証する
  - [ ] `#line` の異常値に専用診断IDを追加し、`DIAG_ERR_PREPROCESS_GENERIC` 依存を減らして誤検知切り分けを容易にする
  - [ ] `#include` 正規化後パス（`./`, `//` 畳み込み結果）を使った循環検出・`pragma once` 判定が常に一致する回帰テストを強化する
