...
### バグ修正（既存機能の不具合）
- [ ] ポインタ加算のスケーリングが効かない（`int *p = arr; p = p + 2;` で `*(p)` が `arr[0]` を返す。`arr[2]` を期待）
  - `p + n` は `p + n * sizeof(*p)` バイト進むべきだが、バイトスケーリングが未実装の可能性
  - 配列添字 `arr[i]` は正常に動作するため、ND_ADD のポインタ加算パスに限局した問題
- [ ] グローバル配列（`int g[3];`）へのアクセスで SIGSEGV が発生する
  - グローバル変数のスカラー値は正常。配列として `.bss` / `.data` にレイアウトする処理が未実装の可能性
- [ ] 多次元配列の初期化子（`int a[2][3] = {{1,2,3},{4,5,6}};`）が正しい値を返さない
  - `a[1][2]` が 6 であるべきところ 1 を返す

### パーサー機能拡張
- [ ] フレキシブル配列メンバー（C99/C11 6.7.2.1）を受理する
  - `struct flex { int len; int data[]; };` が現在パースエラー（E2007: 配列サイズの整数リテラルを要求）
  - パーサーで `[]` を認識し、サイズ 0 の配列として扱う対応が必要
- [ ] 複合リテラルのスコープ規則を厳密化する
...
- [ ] 暗黙の関数宣言を警告する（C11 では暗黙の `int` 戻り値型は廃止）
- [ ] 到達不能コードの警告を追加する（`return` 後のコード等）
- [ ] 初期化されていない変数の使用を警告する
