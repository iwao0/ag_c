...
- [ ] セキュリティ追加候補（2026-03-24 追記2）
  ...
  - [ ] [P2] include 探索パスの許可リスト方針（将来 `-I` 相当追加時）を定義し、境界外拒否テストを先行整備する
  - [x] [P2] エラーパスでのメモリリーク/Use-after-free を ASan/LSan で検出するテストジョブを追加する
  - [ ] [P2] 入力中に NUL バイトを含むバイナリ寄りソースで診断が破綻しないことを検証する
  - [ ] [P3] 診断メッセージの情報漏えい（絶対パス・環境依存情報）を抑制する方針を明文化しテスト化する
  - [ ] [P3] 巨大入力系テストに実行時間上限と失敗時の原因分類ログを追加し、CIハング耐性を上げる
