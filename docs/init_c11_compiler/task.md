...
#### プリプロセッサ
- [ ] プリプロセッサのトークンコピー (`calloc` + 手動コピー) をアリーナ化する
  - `preprocess.c` でトークン複製が約15箇所あり、各 `calloc` に malloc ヘッダが付く
  - トークナイザの既存アリーナ (`tk_allocator_calloc`) を活用すればヘッダ削減可能
- [ ] `hideset_t` をアリーナ割り当てに変更する
  - マクロ展開のたびに `calloc(1, sizeof(hideset_t))` + `my_strndup` が発生
  - 短命オブジェクトのため、専用アリーナで一括解放すると効率的
- [ ] `macro_t` の `params` 配列を固定長バッファで代替する
  - 大半のマクロはパラメータ数が少ない（0〜4個）ため、小さい固定配列＋フォールバック方式でヒープ確保を削減

#### コードジェネレータ
- [ ] `cg_emitf` の毎回 `malloc` + `free` を固定バッファ化する
  - 呼び出しごとに `malloc(need+1)` → `vsnprintf` → `free` しており、アセンブリ行は短い（通常 < 128B）
  - スタック上の固定バッファ（例: 256B）で十分なケースが大半。超過時のみ `malloc` にフォールバック
- [ ] `label_map_t` をアリーナ割り当てに変更する
  - 関数ごとに `calloc` で確保し `clear_label_map` でリスト破棄するが解放していない
  - パーサのアリーナまたは関数スコープのアリーナに載せればリーク解消＋ヘッダ削減

#### トークナイザ
- [ ] `token_t` base の `at_bol` / `has_space` をビットフィールド化する
  - 現在 `bool`（1B）× 2 だがパディングで計8B消費。`:1` ビットフィールドにすれば `kind` と同じ4Bワードに収まる
- [ ] `token_t` base の `file_name` ポインタを intern（共有）する
  - 同一ファイル内の全トークンが同じ `file_name` 文字列を持つが、各トークンに8Bポインタがある
  - ファイル名テーブル（インデックス = `uint16_t`）で2Bに縮小可能
