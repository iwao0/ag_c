...
- [x] 優先度P1: `tokenizer_context` 実装に着手する
  ...
  - [x] 進捗（2026-03-23）: `stmt.c` の宣言系処理（`typedef`/`_Static_assert`/tag宣言の入口）を `tk_get_current_token` / `tk_set_current_token` 経由へ移行した
  - [x] 進捗（2026-03-23）: `stmt.c` の `struct/union` メンバレイアウト解析でも `token` 直接参照をやめ、`tk_get_current_token` / `tk_set_current_token` 経由に寄せた
  - [x] 進捗（2026-03-23）: `parser.c` のトップレベル修飾子/`_Static_assert` 処理を `tk_get_current_token` / `tk_set_current_token` 経由へ移行した
  - [x] 進捗（2026-03-23）: `parser.c` のトップレベル入口ループ（pragma/static_assert/typedef/tag判定）と `_Atomic` 型指定子処理を `tk_get_current_token` / `tk_set_current_token` 経由へ移行した
  ...
  - [x] 進捗（2026-03-23）: `expr.c` に `curtok`/`set_curtok` ヘルパを導入し、型修飾子スキップと配列抽象宣言子サイズ診断の `token` 参照を `curtok` ベースへ置換した
...
- [x] 優先度P1: Tokenizer内部のグローバルカーソルを完全に文脈化する
  ...
  - [x] 進捗（2026-03-23）: `tk_get_current_token_ctx`/`tk_set_current_token_ctx` を追加し、`preprocess` のトークンカーソル退避/復元も context 明示APIへ移行した
...
- [x] 優先度P1: `tokenizer_context_t` への状態集約を完了する
  ...
  - [x] 進捗（2026-03-23）: `main` の入力ファイル名設定を `tk_set_filename_ctx(tk_get_default_context(), ...)` へ移行し、`tokenizer.c` 内のファイル名/監査offset参照も runtime context 直接参照へ寄せた
...
- [x] [P2] `test/bench_tokenizer.c` の hotpath ケースを `TOKENIZER_BENCH_MODE` 環境変数で選択可能にし、局所計測の反復を短縮する
  - [x] 進捗（2026-03-24）: `TOKENIZER_BENCH_MODE=all|cases|hotpath|scanner|literals|punctuator` を追加し、局所計測時の不要ケースを省略可能にした
  - [x] 目的: `scanner`/`literals`/`punctuator` の単体反復時に不要ケースを省く
- [x] [P3] `tokenizer_hotpath_daily.csv` の列説明を `tokenizer_perf_report.md` に追記し、運用時の読み違いを防ぐ
...
- [x] セキュリティ追加候補（2026-03-23 追補）
  - [x] `ag_c` 起動引数がディレクトリ（例: `.`）の場合に、入力読み込み失敗として診断されることを E2E で検証する
  - [x] `ag_c` 起動引数が権限なしファイル（`chmod 000`）の場合に、入力読み込み失敗として安全に終了することを検証する
  - [x] 極端に大きい単一行入力を与えたとき、メモリ/時間面で暴走せず診断終了できることを回帰テスト化する
  - [x] `#include <...>` 形式でも `../` など許可外参照が拒否されることを異常系テストで固定する
  - [x] `#pragma once` と正規化パス（`a//b.h` と `a/b.h`）が混在する循環ケースで判定一貫性を検証する
- [x] セキュリティ追加候補（2026-03-24 追加）
  - [x] `#include` の自己参照（同一ヘッダが自分自身を `#include`）で、深さ上限まで暴走しないことを検証する
  - [x] `#include` パスに末尾スラッシュや `.` 単体（例: `"build/"`, `"."`）を与えたときの診断を固定する
  - [x] マクロ引数の深いネスト（`F(F(F(...)))`）で展開上限に達したときの診断を固定する
  - [x] `##` 連結でトークン境界が曖昧なケース（数値+識別子、演算子連結）を網羅する
  - [x] `#line` のファイル名に制御文字・長大文字列を与えたときの診断安定性を検証する
  - [x] UTF-8 不正バイト列を含むソース入力で tokenizer 診断が安定して出ることを検証する
  - [x] `ag_c` 引数なし / 引数2個以上の usage エラーを E2E で固定する
  - [x] 複数 `ag_c` 同時起動時にテスト用一時ファイル・ログが衝突しないことを検証する
- [x] セキュリティ追加候補（2026-03-24 追記2）
  - [x] [P1] シンボリックリンク経由の `#include` 逸脱（許可外参照）を `realpath` ベースで防止できることを検証する
  - [x] [P1] include 読み込みの TOCTOU（検査後差し替え）リスクを洗い出し、対策方針と回帰テストを整備する
  - [x] [P1] 診断IDの一貫性監査を行い、`DIAG_ERR_*_GENERIC` 依存を段階的に専用IDへ分解する
  - [x] [P2] include 探索パスの許可リスト方針（将来 `-I` 相当追加時）を定義し、境界外拒否テストを先行整備する
  - [x] [P2] エラーパスでのメモリリーク/Use-after-free を ASan/LSan で検出するテストジョブを追加する
...
- [x] セキュリティ追加候補（2026-03-23 追加3）
  - [x] [P1] 将来 `-I` 相当オプション追加時の allowlist 境界（`..` / symlink / `//` / 絶対パス混在）を事前に異常系テストで固定する
  - [x] [P1] 生成ログ・一時ファイルの命名衝突/予測可能パス利用を棚卸しし、並列実行・TOCTOU 耐性テストを追加する
  - [x] [P2] 診断メッセージの情報漏えい再点検（cwd / ユーザー名 / 実 tmp パス等）を網羅テスト化する
  - [x] [P2] 入力サイズ上限超過時の診断ID・終了コード・ログ形式を Tokenizer/Preprocess/Parser で統一し回帰テスト化する
  - [x] [P2] 再帰/ネスト系 DoS 耐性（`#include` 深さ・マクロ再帰・式ネスト・括弧ネスト）の上限値を明文化し、各上限到達ケースを追加する
    - [x] 上限値を明文化（`#include` 深さ: 64、マクロ展開ステップ: 20000、式ネスト: 1024、括弧ネスト: 1024）
    - [x] 上限到達テストを追加（include深さ `E1004`、マクロ展開 `E1029`、式/括弧ネストは parser 失敗診断で固定）
  - [x] [P2] 異常バイト列（UTF-8 不正 / NUL / 制御文字）をソース本体・`#line`・`#include`・マクロ引数で横断検証する

- [x] セキュリティ追加候補（2026-03-23 追加4）
  - [x] [P1] `realpath()` 失敗時（例: symlink loop/ELOOP）に include 処理が安全に失敗し、診断が安定することを回帰テスト化する
  - [x] [P2] `#line` ファイル名の UTF-8 妥当性ポリシー（許可/拒否）を明文化し、診断表示崩れ防止テストを追加する
  - [x] [P2] 将来の中間ファイル/キャッシュ導入を見据えた一時ファイル命名ポリシー（PID/乱数/隔離）を文書化する

- [x] セキュリティ追加候補（2026-03-23 追加5）
  - [x] [P1] `#include` 読み込み失敗を errno 理由別（not found / permission denied / symlink loop）に診断ID分離し、回帰テストを更新する
  - [x] [P2] プリプロセッサ `#if` 条件式の計算量上限（トークン数/評価ステップ）を定義し、上限到達ケースを追加する
  - [x] [P2] Parser の「幅方向」上限（巨大初期化子要素数・巨大宣言列）を定義し、メモリ圧迫系の回帰テストを追加する

- [x] セキュリティ追加候補（2026-03-23 追加6）
  - [x] [P1] `#include` の TOCTOU 最終対策として `open` 後 `fstat` で実体検証（種別/サイズ境界）を追加し、差し替え耐性テストを強化する
  - [x] [P1] 診断メッセージの情報露出（絶対パス/ユーザー名/tmp実パス）を全診断ID横断で再監査し、露出テストを追加する
  - [x] [P2] 巨大入力上限超過時の終了コード・診断形式・ログ量を Tokenizer/Preprocess/Parser で完全統一する
  - [x] [P2] 失敗系テストを含むリソースリーク監査（tmp/symlink/fd）の常設テストを追加する
  - [x] [P2] `#if`/マクロ/初期化子を中心とした短時間ファジングをCIへ組み込み、クラッシュ/ハング回帰を検出する

- [x] セキュリティ追加候補（2026-03-24 追加7）
  - [x] [P1] `#include` 読み込みで `open(..., O_NOFOLLOW)` を導入し、最終要素 symlink を拒否する回帰テストを追加する
  - [x] [P2] `ulimit` 制約下（低メモリ/低fd）で失敗時の終了コード・診断形式・後始末が崩れないことをE2Eで検証する
  - [x] [P2] 診断出力時に制御文字/Bidi制御文字をエスケープし、ログ破壊・表示欺瞞を防ぐテストを追加する

- [ ] セキュリティ追加候補（2026-03-24 追加8）
  - [ ] [P1] `#include` の中間ディレクトリ symlink すり抜けを防ぐため、`openat` 連鎖 + `fstatat` 監査を導入し回帰テストを追加する
  - [ ] [P2] 診断メッセージの出力量に上限を設け、巨大/悪意入力時の stderr 肥大化を防ぐテストを追加する
  - [ ] [P2] fail-path を連続実行しても fd/メモリリークが蓄積しないことをCIで機械検証する
  - [ ] [P2] `ASan/UBSan`（可能なら `LSan`）の定期ジョブを追加し、未定義動作・潜在クラッシュを早期検出する
  - [ ] [P3] セキュリティ強化ビルド（`-fstack-protector-strong`, `-D_FORTIFY_SOURCE=2` 相当）をCIで常時検証する
...
- [ ] `type_name` をキャスト・sizeof・_Generic で統一する（C11 §6.7.7）
  ...
  - 進捗（2026-03-24）: cast の type-name でも `int (*(*)(void))[3]` を受理し、`_Generic((int (*(*)(void))[3])0, ...)` の制御式を受理
  ...
  - 進捗（2026-03-24）: `sizeof(int (*(*[2])(void))[3])` のような「配列 of 関数ポインタ（配列ポインタ返却）」抽象宣言子を受理し、型サイズ計算に反映
...
- [ ] [P1] 宣言子パース経路を段階的に一元化する（`parser.c` / `decl.c` / `stmt.c`）
  ...
  - 進捗（2026-03-24）: `parser.c` の仮引数 `decl_spec` 解析を `parse_param_tag_decl_spec()` / `parse_param_scalar_decl_spec()` に分割し、`parse_param_decl_spec()` から tag/スカラー分岐の詳細処理を分離
  - 進捗（2026-03-24）: `parser.c` のトップレベル object 宣言登録を `register_toplevel_object_from_declarator()` / `consume_toplevel_extern_initializer_if_any()` へ分離し、extern/非extern の後段処理を整理
  ...
  - 進捗（2026-03-24）: `parser.c` の `parse_toplevel_declaration_like()` から宣言開始条件を `is_toplevel_decl_like_start()` に抽出し、トップレベル宣言入口の分岐責務を軽量化
  ...
  - 進捗（2026-03-24）: `parser.c` の `parse_toplevel_decl_spec()` から `typedef` 記憶クラス消費を `consume_toplevel_typedef_storage_class()` へ抽出し、宣言仕様子入口の責務を分離
  - 進捗（2026-03-24）: `parser.c` のトップレベル通常宣言/typedef宣言で重複していた先頭 `*` 連鎖消費を `parse_toplevel_pointer_prefix()` へ抽出
  - 進捗（2026-03-24）: `parser.c` の `parse_toplevel_decl_after_type()` を `parse_toplevel_typedef_declaration_stmt()` / `parse_toplevel_object_declaration_stmt()` に分割し、宣言種別ごとの文末処理責務を分離
  - 進捗（2026-03-24）: `parser.c` のトップレベル宣言子列上限チェックを `guard_toplevel_declarator_count()` へ抽出し、宣言子ループの責務を軽量化
  ...
  - 進捗（2026-03-24）: `parser.c` の `parse_decl_name_recursive()` から識別子取得+必須診断を `consume_decl_ident_or_error()` に抽出し、宣言子名消費責務を分離
  - 進捗（2026-03-24）: `parser.c` の `parse_decl_name_recursive()` から括弧付き宣言子後の `()` サフィックス走査を `consume_toplevel_paren_decl_func_suffixes_if_any()` に抽出
  ...
  - 進捗（2026-03-24）: `parser.c` の object/typedef 宣言子先頭解析を `parse_toplevel_declarator_head(base_is_ptr, require_name)` に統合し、宣言子ヘッド読み取り責務を共通化
  - 進捗（2026-03-24）: `parser.c` の `define_toplevel_typedef_from_declarator()` から typedef登録API呼び出しを `register_toplevel_typedef_name()` に抽出
  - 進捗（2026-03-24）: `parser.c` の `parse_one_toplevel_typedef_declarator()` から「宣言子ヘッド→typedef定義」連結を `apply_toplevel_typedef_from_head()` に抽出
  ...
  - 進捗（2026-03-24）: `parser.c` の object/typedef 宣言子列ループを `parse_toplevel_declarator_list_with_apply(base_is_ptr, apply)` に統合し、宣言子反復経路を一元化
  - 進捗（2026-03-24）: `parser.c` の `parse_toplevel_decl_after_type()` を `parse_toplevel_declarator_stmt(base_is_ptr, apply)` 経由へ統合し、typedef/通常宣言で同一の文処理経路を共有
  - 進捗（2026-03-24）: `stmt.c` の struct/union メンバ宣言ループでも宣言子先頭解析を `parse_stmt_member_decl_head()` へ抽出し、`parser.c` と同じ「head→適用」構造へ寄せた
  - 進捗（2026-03-24）: `stmt.c` の `parse_member_decl_name_recursive_stmt()` から関数サフィックス `()` 消費を `consume_stmt_member_func_suffixes()` に抽出し、`parser.c` 側の関数サフィックス処理分離方針へ揃えた
  ...
  - 進捗（2026-03-24）: `parser.c` に `psx_consume_pointer_prefix()` を公開し、トップレベルと `stmt.c` の宣言子再帰/typedef経路で重複していた `*` 連鎖 + pointer qualifier 消費を同一実装へ統一
...
- [x] `_Static_assert` を `declaration` の一種にする（C11 §6.7）
  ...
  - 進捗（2026-03-24）: ブロックスコープでも `parse_decl_like_stmt()` が `_Static_assert` を `psx_decl_parse_declaration()` へ委譲するようにし、`decl.c` 側で `static_assert-declaration` を処理する構造へ整理
...
- [ ] 配列宣言子で `assignment-expression` を許容する（C11 §6.7.6.2）
  ...
  - 進捗（2026-03-24）: ローカル `typedef` 宣言でも `typedef int A[];`（不完全配列型）を受理し、回帰テストで固定化
