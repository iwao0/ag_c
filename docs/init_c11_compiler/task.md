...
- [ ] セキュリティ追加候補（2026-03-24 追加）
  - [ ] `#include` の自己参照（同一ヘッダが自分自身を `#include`）で、深さ上限まで暴走しないことを検証する
  - [ ] `#include` パスに末尾スラッシュや `.` 単体（例: `"build/"`, `"."`）を与えたときの診断を固定する
  - [ ] マクロ引数の深いネスト（`F(F(F(...)))`）で展開上限に達したときの診断を固定する
  - [ ] `##` 連結でトークン境界が曖昧なケース（数値+識別子、演算子連結）を網羅する
  - [x] `#line` のファイル名に制御文字・長大文字列を与えたときの診断安定性を検証する
  - [ ] UTF-8 不正バイト列を含むソース入力で tokenizer 診断が安定して出ることを検証する
  - [ ] `ag_c` 引数なし / 引数2個以上の usage エラーを E2E で固定する
  - [ ] 複数 `ag_c` 同時起動時にテスト用一時ファイル・ログが衝突しないことを検証する
- [ ] セキュリティ追加候補（2026-03-24 追記2）
  - [ ] [P1] シンボリックリンク経由の `#include` 逸脱（許可外参照）を `realpath` ベースで防止できることを検証する
