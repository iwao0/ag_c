...
## 一時ファイル命名ポリシー（将来の中間ファイル/キャッシュ向け）
- 予測可能な固定名（例: `/tmp/ag_c_cache`）は使用しない。
- 一時ファイルは `mkstemp` 相当で作成し、テンプレートは `ag_c_<purpose>_XXXXXX` 形式を基本とする。
- 共有領域（`/tmp` など）に置く場合は、ファイル作成時に排他作成を必須とし、既存ファイル上書き経路を作らない。
- 実行単位の隔離を優先し、必要に応じて `build/<pid>/...` などプロセス単位ディレクトリを使う。
- ログ/診断には一時ファイルの実パスをそのまま表示しない（表示時は basename か論理名に縮約する）。
- 異常終了時も残骸が長期残留しないよう、生成側でクリーンアップ方針（終了時削除/次回起動時回収）を定義する。
