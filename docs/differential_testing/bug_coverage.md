...
| 同梱 `<complex.h>` (I/creal/cimag/conj、算術) | 🔧 | stdheader/complex_ops | _Complex brace 初期化・複素数 compound literal で I={0,1}、creal/cimag は __real__/__imag__ ベース (rvalue 可)。算術はネイティブ。値渡し無し / cabs 等 math 依存は非提供 |
...
| 同梱 `<stdatomic.h>` (単一スレッド版) | 🔧 | stdheader/stdatomic_ops | 型/enum/マクロ/load/store/fetch_add 等。exchange/fetch_or/fetch_and は新値返却 (仕様逸脱明記) |
