...
| グローバル funcptr **配列** `gops[i]()` の fp 戻り | ⬜ | — | スカラ global は対応済み。配列は未確認 |
...
## 未チェックの着手候補（⬜ を埋める）
- グローバル funcptr 配列の fp 戻り。
- 関数呼び出しをまたぐ型変換（promotion）全般の再点検。
- ビットフィールドの符号・パッキングの網羅（既存は long bitfield 中心）。
- `_Atomic` / `volatile` / `restrict` 修飾の挙動。
- 文字列・ワイド文字リテラル、エスケープの端ケース。
