...
| **多次元 `static` ローカル配列の永続化** `static int a[2][3]` / `static double d[2][2]` / `typedef int M[2][3]; static M m` | 🔧 | static_local_multidim_array, static_local_typedef_multidim_array | 1D static local 配列は lowering されるが、多次元 suffix は `try_lower_static_local_array` が `[` の連続を見て対象外にし、通常 auto 多次元配列として frame に置かれていた。呼び出し間で値が保持されず、Wasm IR でも alloca になっていた。多次元定数 suffix を peek/consume して consumed lowering へ渡し、global/alias に outer/mid/extra stride を保存、`ND_ADDR(ND_GVAR)` へ伝播。続き229で static local alias の unsigned pointee 伝播漏れと FP 配列の `fp_kind` / `init_fvalues` 漏れを修正し、unsigned char/short/long/double も網羅。続き230で typedef-array local 登録より前に static typedef-array lowering を追加し、`static M m` も stack local に落ちないようにした |
