...
| 変則形 `*(t+1)[0]`（pointer-to-2D-array に subscript+deref 混在） | ⚠️ | — | SIGBUS(agc=138)。`*((t+1)[0])` と解釈、codegen がアドレス誤計算。`(*(t+1))[0]`/`t[1][0]` は正常 |
| typedef 配列へのポインタの算術 stride `typedef int R[3]; R *p; p+1` | ⚠️ | — | pointee を int(4) 扱いし stride 1。直書き `int(*p)[3]` は正常。typedef 配列の dims×elem が pointee に伝播せず。`p[1][0]` も誤 |
| array-of-struct の連鎖 designator `[i].field=` の初期化 | ⚠️ | — | `struct R a[2]={[1].id=5}` が E2006 (`=` 期待 `.`)。index designator 後に member selector を再パースしない。さらに designated 要素の残メンバが 0 補完されない (集約初期化の共通根) |
| struct 配列の no-brace 部分初期化 `struct R a[2]={0}` | ⚠️ | — | スカラ列が struct 要素の途中で尽きると残メンバが garbage (`a[0].val` 未補完)。`{{0,0},{0,0}}`/`{1,2,3,4}`/`[i]={...}`+未指定要素は可。頻出イディオム |
| ネスト struct を含む配列の brace/0補完 | ⚠️ | — | `struct{struct inner in;int n;} a[2]={[1]={{1,2},3}}` 等が ir_build_module failed。要素が struct メンバを含むと 0 補完/brace 経路が壊れる |
...
| `_Generic` が `long` と `long long` を同一視 | ⚠️ | — | `_Generic(0LL, long:.., long long:..)` で long 側にマッチ。両 8B のため rank 未区別 |
| `_Generic` が引数違いの関数ポインタを同一視 | ⚠️ | — | `int(*)(int)` と `int(*)(int,int)` を区別せず最初の funcptr assoc にマッチ |
| enum 値・算術 | ✅ | (probe q3) | |
