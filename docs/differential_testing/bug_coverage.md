...
| ファイルスコープのポインタ要素配列複合リテラル `(int *[]){&g,...}` / `(struct S *[]){...}` / typedef 経由 `(IP[]){...}` | 🔧 | file_scope_ptr_from_array_compound, test_wasm32_object: global_compound_literal_inner_ptr_data_reloc, typedef_compound_literal_inner_ptr_data_reloc | cast parser がポインタ型後続の `[]` を非ポインタ型だけに限定し、`(int *[]){...}` を compound literal と認識できず E3064。hidden global/lvar に「要素がポインタ」の metadata を載せ、`**ptrs` / `node_ptrs[i]->value` と Wasm object の data initializer 内 `&g` / struct pointer relocation を固定 |
