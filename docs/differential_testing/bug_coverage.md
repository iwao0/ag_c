...
最終更新: 2026-06-30（Wasm object compound literal address relocation fixture まで）
...
| ファイルスコープのポインタ要素配列複合リテラル `(int *[]){&g,...}` / `(struct S *[]){...}` | 🔧 | file_scope_ptr_from_array_compound, test_wasm32_object: global_compound_literal_inner_ptr_data_reloc | cast parser がポインタ型後続の `[]` を非ポインタ型だけに限定し、`(int *[]){...}` を compound literal と認識できず E3064。hidden global/lvar に「要素がポインタ」の metadata を載せ、`**ptrs` / `node_ptrs[i]->value` と Wasm object の data initializer 内 `&g` relocation を固定 |
