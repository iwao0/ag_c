...
## 未チェックの着手候補（⬜ を埋める）
- **既存バグ (発見済み・未修正、調査メモあり)**: cast 制御式の深いネスト型 `_Generic`
  （`_Generic((int(*(*)(void))[3])0, int(*(*)(void))[3]:.., default:..)`）が、特定の組合せで
  「':' が必要 (実際 '(')」E2006 を出す。**今回の `_Generic` 作業より前の 72b23c2 でも再現**する
  既存バグで、型シグネチャ照合とは独立。切り分け結果:
  - トリガ条件 = ①複雑なグローバル funcptr/ネスト宣言 + ②複数の `assert(_Generic(...))` +
    ③末尾に cast 制御式の深いネスト `_Generic`、の同居。①無し(8 assert)→OK、②が 2 個→OK、
    `assert` でなく stringize しない自作マクロ→OK、`assert` の `#expr` 文字列化が必須。
  - ストリーミングのチャンク再利用は**無関係**（`tk_allocator_recyc_on_cursor` を無効化しても再現）。
  - 根本原因は未特定（`assert` の `#expr` 文字列化の蓄積 + 複雑グローバル + cast `_Generic` の
    パーサ状態干渉と推測）。稀・パースエラー（誤コンパイルではない）で実害は小さく、深掘りコストが
    見合わないため一旦保留。
