...
| 変則形 `*(t+1)[0]`（pointer-to-2D-array に subscript+deref 混在） | ⚠️ | — | SIGBUS(agc=138)。`*((t+1)[0])`。根本: ポインタ算術 `t+1` の ND_ADD が plain node_t で pointer-to-array の outer_stride/array メタを持たず、make_subscript_scaled_offset が ND_ADD base を LVAR/DEREF/GVAR 分岐で拾えず要素を int(4) 扱い→ `(t+1)[0]` が配列へ decay せずスカラ load→外側 `*` が値をアドレス deref。`(*(t+1))[0]`/`t[1][0]` は base が lvar/deref で正常。修正は subscript stride 機構かポインタ算術ノードのメタ伝播 (コア・要注意) |
| typedef 配列へのポインタの算術 stride `typedef int R[3]; R *p; p+1` | 🔧 | typedef_array_pointer_stride | is_pointer+td_array 分岐を追加し outer_stride=配列全体を設定。1D は p+1/p[i][j] 修正。2D typedef ポインタの深い添字 q[i][j][k] は mid_stride 未伝播で別途 ⚠️ |
| 2D 以上の typedef 配列ポインタの深い添字 `m23 *q; q[i][j][k]` | ⚠️ | — | 2 段目が mid_stride を使わず誤値。直書き `int(*q)[2][3]` は正常。typedef 経路の多段ストライド伝播が不完全 |
