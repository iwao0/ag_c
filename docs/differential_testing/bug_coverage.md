...
最終更新: 2026-06-20（typedef 自体が配列へのポインタ `typedef int (*PA)[3]` の局所使用 まで）
...
| 〃 を**グローバル変数**で使用 `typedef int (*PA)[3]; PA gp` | ⚠️ | — | E3064。これは pointer-to-array 固有でなく、ポインタ typedef 全般 (`typedef int *PI; PI gp;` ですら) がグローバル変数で subscript できない既存の別バグ。直書き `int *gp`/`int(*gp)[3]` は動く。グローバルでのポインタ typedef 解決 (deref/pointer 認識) の修正が前提で、本件とは別スコープのため見送り |
