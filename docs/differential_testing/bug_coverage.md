...
| struct の先頭 aggregate funcptr メンバと `{0}` 初期化 | 🔧 | struct_funcptr_zero_init | `struct Holder h = {0};` で先頭メンバが `struct Ops ops[2]` のとき、明示 `0` が 16B aggregate 代入として IR に渡り E4007。struct 全体の pre-zero-fill 後、terminal `0` は no-op として消費し、synthetic lvar の `tag_kind` 既定値も `TK_EOF` に初期化。Wasm object scan も対象 |
