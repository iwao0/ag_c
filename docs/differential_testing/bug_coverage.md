...
| array-of-struct の連鎖 designator `[i].field=` の初期化 | ⚠️ | — | `struct R a[2]={[1].id=5}` が E2006 (`=` 期待 `.`)。`[i]={...}` は可。index designator 後に member selector を再パースしない |
| sized array 複合リテラルのアドレス `&(int[N]){...}` | ⚠️ | — | `int(*p)[1]=&(int[1]){99}` が ir_build_module failed。scalar/struct 複合リテラルの `&` は可 |
