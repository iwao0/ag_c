...
最終更新: 2026-06-21（複数 TU / extern・static リンケージ、_Generic 複雑派生型の照合まで）
...
| 多段ポインタの fp pointee `double **p; **p` / `(*pp)[i]` 配列 decay 添字 | ⚠️ | — | **直書きでも local/global 双方で壊れる**別の既存バグで、多段ポインタ typedef 固有ではない。fp_kind の多段ポインタ伝播 / 多段ポインタの配列 decay 添字という中核 codegen の穴が前提のため別件として見送り |
...
| `_Generic` 深いネスト型 (`(int(*(*)(void))[3])`) の同一型マッチ | 🔧 | generic_complex_derived_type | 構造的フィールドでは区別不能だったため、型を正規化トークン文字列にして照合 (control が単一識別子のとき宣言時に名前で記録)。control/assoc 双方に sig があれば strcmp、片方のみは従来照合 (加算的) |
...
| `_Generic` が引数違いの関数ポインタを同一視 | 🔧 | generic_complex_derived_type | `int(*)(int)` と `int(*)(int,int)`、`int(*)(int)` と `double(*)(int)` を区別。型を正規化トークン文字列化して比較するので引数の個数・型・戻り型まで照合する。制御式が単一識別子の局所変数のとき有効 (キャスト/グローバルは今後) |
...
## 未チェックの着手候補（⬜ を埋める）
- `_Generic` の制御式がキャスト/グローバル変数のときの複雑派生型照合（局所変数は対応済み。
  型シグネチャを cast/gvar 経路へも配線すれば拡張可能）。
- 複数 translation unit / extern リンケージの差分（一巡済み: 上表「リンケージ / 複数 TU」。
  extern 各種は green、ファイルスコープ static の内部リンケージ違反を発見・修正）。
