...
最終更新: 2026-06-30（Wasm object global string offset data relocation fixture まで）
...
| Wasm object の global string offset pointer data relocation | 🔧 | test_wasm32_object: global_string_offset_data_reloc | file-scope `const char *p="abc"+1` と pointer 配列要素 `"de"+1` が `reloc.DATA` の `R_WASM_MEMORY_ADDR_I32` addend (`<.LC*>+0x1`) 付きになることを固定 |
