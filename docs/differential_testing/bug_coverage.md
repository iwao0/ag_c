...
| **Wasm 制御フロー越し関数ポインタ呼び出し** `if(1) g=set7; g(&x);` / `ops.f(&x)` | 🔧 | wasm32_*_funcptr_control_flow_store, wasm_nonvoid_indirect_unused_result (続き116/239) | global/struct member の関数ポインタが制御フロー越しに上書きされると、Wasm emitter が callee 名を逆引きできず、void call と非 void unused-result call を区別できず E4008。関数ポインタ型に `funcptr_ret_is_void` を保存し、typedef/local/global/tag member/`node_mem_t` から `ND_FUNCALL.is_void_call` と `IR_CALL.is_void_call` へ伝播。続き239で非 void unknown indirect の未使用結果も `IR_CALL.dst.type` から typeuse を組み、`drop (call_indirect ... (result T))` として出せるようにした |
