...
最終更新: 2026-06-20（多段ポインタ typedef `typedef int **PP` の局所/仮引数使用 まで）
...
| 〃 を**グローバル変数**で使用 / `(*pp)[i]` 添字 | ⚠️ | — | これらは**直書き `int **gp` グローバル / `int **p; (*p)[i]` でも同様に壊れる**別の既存バグで、typedef 固有ではない。グローバル多段ポインタ deref・多段ポインタの配列 decay 添字という中核 codegen の穴が前提のため、多段 typedef とは別件として見送り |
