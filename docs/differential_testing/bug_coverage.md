...
- **修正 (続き24)**:
  ...
  - グローバル `struct P *parr[3]; parr[i]->x` (struct ポインタ配列の subscript + ->)
    ...
    emit を scalar 経路に流す。
- **残課題 (次セッション)**:
  - VLA `sizeof(arr)` ランタイム計算: `int arr[sz]` の sizeof が garbage (本来 sz*4)。
    C11 6.5.3.4 の VLA sizeof のランタイム評価が未実装。
  - グローバル struct ポインタ配列の init slot 計算: `struct P *parr[3] = {&pts[0], ...}`
    で parr[1]/parr[2] のシンボル+offset が誤値 (psx_gbrace_flat の child_at が
    is_tag_pointer 配列を struct 単位で展開してしまう)。続き24 fixture は parr[0] のみ網羅。
  - `(*pp)->x` (struct P** の単独 deref からの ->): tag info carry 不足。
