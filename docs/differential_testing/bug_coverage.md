...
## 未チェックの着手候補（⬜ を埋める）
- **既存バグ (発見済み・未修正)**: 深いネスト型 assoc の `_Generic` パースが、複雑なグローバル
  funcptr 宣言 + 複数の `_Generic`（cast 制御式を含む）と同一 TU に同居すると誤パースする
  （`_Generic((int(*(*)(void))[3])0, int(*(*)(void))[3]:.., default:..)` で「':' が必要 (実際 '(')」
  E2006）。`_Generic` 複雑派生型の作業 (5632b86/1cbf0cb/cc1e788) より前の **72b23c2 でも再現**する
  既存バグで、今回の型シグネチャ照合とは独立。assoc のネスト宣言子パーサ
  (try_parse_assoc_abstract_declarators 周辺) が何らかの状態と干渉していると思われる。単独の
  ネスト assoc は通り、特定の組合せでのみ顕在化。実用は稀。
